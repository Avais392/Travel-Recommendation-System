...
class BusinessUserController extends Controller
{
    public function pageLoad( $id = null )
    {
        ...
        return view("BusinessUser", [ 'user' => $loggedInUSer, 'isUserLoggedIn' => $isUserLoggedIn, 'isAlreadyLoggedInUser' => $isAlreadyLoggedInUser ]);
    }
    ...
    public function addNewTravelOption( Request $req )
    {
        $this->validate($req, [
            'title' => 'required',
            'travelType' => 'required|exists:travel_types,id',
            'description' => 'required',
            'destination' => 'required|exists:destinations,id',
            'attraction' => ['nullable', 'exists:attractions,id']//'exists:attractions,id'
        ]);


        $travelInfo = new TravelInfo();

        $travelInfo->destinationID = $req[ "destination" ];
        $travelInfo->attractionID = $req[ "attraction" ];
        $travelInfo->description = $req[ 'description' ];
        $travelInfo->travelTypeID  = $req[ "travelType" ];

        $travelInfo->save();


        return redirect()->back()->with([ 'successMessage' => 'Travel Info created successful!' ]);
    }
